...
public class CampaignServiceImpl implements CampaignService {
  ...
  public void deleteCampaign(Long id) {
    ...
    // 2. 소프트 삭제 실행
    // CommonEntity의 delete() 메소드에서 useYn='N', deleteYn='Y' 처리를 수행합니다.
    campaignEntity.delete(null); 
  }
  ...
  public UserOutDto addCampaignMember(Long campaignId, Long userId) {
    ...
    CampaignEntity campaign = campaignRepository.findById(campaignId)
      .orElseThrow(() -> new CustomException(RESPONSE_CODE.CAMPAIGN_NOT_FOUND, RESPONSE_MESSAGE.CAMPAIGN_NOT_FOUND, campaignId));
    ...
    if (campaignMemberRepository.existsByCampaignIdAndUserId(campaignId, userId)) {
      throw new CustomException(RESPONSE_CODE.CAMPAIGN_MEMBER_ALREADY_EXISTS, RESPONSE_MESSAGE.CAMPAIGN_MEMBER_CONFLICT, user.getName());
    }
  ...
  public UserOutDto removeCampaignMember(Long campaignId, Long userId) {
    ...
    CampaignMemberEntity member = campaignMemberRepository.findByCampaignIdAndUserId(campaignId, userId)
      .orElseThrow(() -> new CustomException(RESPONSE_CODE.CAMPAIGN_MEMBER_NOT_FOUND, RESPONSE_MESSAGE.CAMPAIGN_MEMBER_NOT_FOUND, userId));
  ...
  public CharacterOutDto addCampaignCharacter(Long campaignId, Long characterId) {
    ...
    CharacterEntity character = characterRepository.findById(characterId)
      .orElseThrow(() -> new CustomException(RESPONSE_CODE.CHARACTER_NOT_FOUND, RESPONSE_MESSAGE.CHARACTER_NOT_FOUND, characterId));

    // 2. 관계 검증: 캐릭터는 오직 하나의 캠페인에만 소속될 수 있음 (1:N)
    // 이미 소속이 있다면 'CAMPAIGN_CHARACTER_ALREADY_EXISTS' 예외를 던집니다.
    if (character.getCampaign() != null) {
      throw new CustomException(RESPONSE_CODE.CAMPAIGN_CHARACTER_ALREADY_EXISTS, RESPONSE_MESSAGE.CAMPAIGN_CHARACTER_CONFLICT, character.getName());
    }
  ...
  public CharacterOutDto removeCampaignCharacter(Long campaignId, Long characterId) {
    ...
    if (character.getCampaign() == null || !character.getCampaign().getId().equals(campaignId)) {
      throw new CustomException(RESPONSE_CODE.CAMPAIGN_CHARACTER_NOT_FOUND, RESPONSE_MESSAGE.CAMPAIGN_CHARACTER_NOT_FOUND, character.getName());
    }
