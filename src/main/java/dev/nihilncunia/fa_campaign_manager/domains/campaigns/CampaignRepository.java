 ...
 * 캠페인 기본 리포지토리
 * 
 * 1. JpaRepository<CampaignEntity, Long>: Spring Data JPA가 제공하는 기본 CRUD
 * 메소드(save, findById, delete 등)를 자동으로 구현해줍니다.
...
public interface CampaignRepository extends JpaRepository<CampaignEntity, Long>, CampaignRepositoryCustom {
  ...
  @org.springframework.data.jpa.repository.EntityGraph(attributePaths = { "sessions", "characters", "members.user" })
  Optional<CampaignEntity> findById(Long id);

  /**
   * 쿼리 메소드(Query Method) 기능:
   * 메소드 이름 자체가 쿼리가 되는 마법 같은 기능입니다.
   * 'existsBy' + 'Name' 이라는 이름을 분석하여 Spring이 자동으로 'SELECT count(id) > 0 FROM
   * campaigns WHERE name = ?' 쿼리를 생성합니다.
   * 
